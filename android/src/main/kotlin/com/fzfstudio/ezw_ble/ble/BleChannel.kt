...
enum class BleMC {
    ...
    fun handle(context: Context, arguments: Any?,  result: MethodChannel.Result) {
        when (this) {
            ...
            BLE_STATE -> {
                return result.success(BleManager.Companion.instance.currentBleState)
            }
            ENABLE_CONFIG -> {
                val jsonMap = arguments as Map<*, *>?
                val config = jsonMap?.toJson<BleConfig>()
                if (config != null) {
                    BleManager.Companion.instance.enableConfig(config)
                }
            }
            START_SCAN -> BleManager.Companion.instance.startScan()
            STOP_SCAN -> BleManager.Companion.instance.stopScan()
            CONNECT_DEVICE -> {
                val jsonMap = arguments as Map<*, *>?
                ...
                if (uuid.isNotEmpty() && sn.isNotEmpty()) {
                    BleManager.Companion.instance.connect(uuid, sn, afterUpgrade = afterUpgrade)
                }
            }
            DISCONNECT_DEVICE -> {
                val uuid = arguments as String? ?: ""
                BleManager.Companion.instance.disconnect(uuid)
            }
            SEND_CMD -> {
                val jsonMap = arguments as Map<*, *>?
                val uuid = jsonMap?.get("uuid") as String? ?: ""
                val data = jsonMap?.get("data") as ByteArray? ?: byteArrayOf()
                BleManager.Companion.instance.sendCmd(uuid, data)
            }
            ENTER_UPGRADE_STATE -> {
                val uuid = arguments as String? ?: ""
                BleManager.Companion.instance.enterUpgradeState(uuid)
            }
            QUITE_UPGRADE_STATE -> {
                val uuid = arguments as String? ?: ""
                BleManager.Companion.instance.quiteUpgradeState(uuid)
            }
            DEVICE_CONNECTED -> {
                val uuid = arguments as String? ?: ""
                BleManager.Companion.instance.setConnected(uuid)
            }
            OPEN_BLE_SETTINGS -> {

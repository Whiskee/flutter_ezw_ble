...
A `physical_connection` step carries `physicalConnectionEvent=connected|disconnected` only after the platform callback passes the existing current peripheral/GATT owner check. This step is context evidence, not a new connection state or success/failure result. Consumers must handle it before normal stage parsing. Logical reset, cancellation and `didFailToConnect` do not set physical timestamps. Platform callbacks filtered after owner teardown remain unknown rather than being attributed to a later owner.

...
void main() {
  test('OTA reboot teardown has a dedicated method-channel contract', () {
    final platform = File(
      'lib/flutter_ezw_ble_platform_interface.dart',
    ).readAsStringSync();
    final method = File(
      'lib/flutter_ezw_ble_method_channel.dart',
    ).readAsStringSync();
    final android = File(
    ...
    expect(platform, contains('disconnectForOtaRecovery'));
    expect(method, contains('"disconnectForOtaReboot"'));
    expect(method, contains('"disconnectForOtaRecovery"'));
    ...
    expect(android, contains('DISCONNECT_FOR_OTA_RECOVERY'));
    ...
    expect(ios, contains('case disconnectForOtaRecovery'));
    expect(ios, contains('expectedSessionGeneration'));
    expect(ios, contains('expectedAttemptGeneration'));
  });

  test(
    'OTA reboot does not reuse the user-cancel path or schedule immediately',
    () {
      final android = File(
      ...
      expect(
        androidMethod,
        isNot(contains('missing epoch-accepted admission')),
      );
      ...
      expect(
        iosMethod,
        contains('attemptGeneration: metadata?.attemptGeneration'),
      );
  ...
  test(
    'iOS OTA recovery uses exact real disconnect without reboot suppression',
    () {
      final ios = File('ios/Classes/ble/BleManager.swift').readAsStringSync();

      final recoveryStart = ios.indexOf('func disconnectForOtaRecovery');
      final recoveryEnd = ios.indexOf('\n    /// 标记/消费 OTA', recoveryStart);
      final recoveryMethod = ios.substring(recoveryStart, recoveryEnd);

      expect(recoveryMethod, contains('return "accepted"'));
      expect(recoveryMethod, contains('return "alreadyDisconnected"'));
      expect(recoveryMethod, contains('return "staleIdentity"'));
      expect(recoveryMethod, contains('return "unavailable"'));
      expect(recoveryMethod, contains('otaWriteQueues.removeValue'));
      expect(recoveryMethod, contains('upgradeStateRegistry.consume'));
      expect(
        recoveryMethod,
        contains('centralManager.cancelPeripheralConnection'),
      );
      expect(
        recoveryMethod,
        isNot(contains('markOtaRebootDisconnectSuppression')),
      );
      expect(recoveryMethod, isNot(contains('handleConnectState')));
      expect(recoveryMethod, isNot(contains('suppressReconnectSchedule')));
    },
  );

  test(
    'Android OTA reboot preserves owner but drops the closed physical GATT',
    () {
      final reconnect = File(
        'android/src/main/kotlin/com/fzfstudio/ezw_ble/ble/BleAutoReconnectSupervisor.kt',
      ).readAsStringSync();

      final detachStart = reconnect.indexOf(
